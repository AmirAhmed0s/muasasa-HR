...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
    ...
    invoices = get_invoices(filters, additional_query_columns)
    invoice_list = []
    if filters.get("territory") and not filters.get("customer_group"):
        for invoice in invoices:
            if invoice["territory"] in filters.get("territory"):
                invoice_list.append(invoice)
    elif not filters.get("territory") and filters.get("customer_group"):
        for invoice in invoices:
            if invoice["customer_group"] in filters.get("customer_group"):
                invoice_list.append(invoice)
    elif filters.get("territory") and filters.get("customer_group"):
        for invoice in invoices:
            if invoice["customer_group"] in filters.get("customer_group") and invoice["territory"] in filters.get(
                "territory"
            ):
                invoice_list.append(invoice)
    ...
    invoice_income_map, invoice_tax_map = get_invoice_tax_map(invoice_list, invoice_income_map, income_accounts)
    ...
    for inv in invoice_list:
        ...
        for account in unrealized_profit_loss_accounts:
            row.update({frappe.scrub(account + "_unrealized"): flt(internal_invoice_map.get((inv.name, account)))})
        ...
        for dimension in accounting_dimensions:
            if filters.get(dimension.fieldname):
                if frappe.get_cached_value("DocType", dimension.document_type, "is_tree"):
                    ...
                    conditions += common_condition + "and ifnull(`tabSales Invoice`.{0}, '') in %({0})s)".format(
                        dimension.fieldname
                    )
                else:
                    conditions += common_condition + "and ifnull(`tabSales Invoice`.{0}, '') in %({0})s)".format(
...
def get_invoice_cc_wh_map(invoice_list):
    ...
    for d in si_items:
        if d.cost_center:
            invoice_cc_wh_map.setdefault(d.parent, frappe._dict()).setdefault("cost_center", []).append(d.cost_center)

        if d.warehouse:
            invoice_cc_wh_map.setdefault(d.parent, frappe._dict()).setdefault("warehouse", []).append(d.warehouse)
