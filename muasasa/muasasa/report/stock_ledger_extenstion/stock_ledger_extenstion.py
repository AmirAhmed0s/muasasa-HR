...
def get_stock_ledger_entries(filters, items):
    item_conditions_sql = ""
    if items:
        item_conditions_sql = "and sle.item_code in ({})".format(", ".join(frappe.db.escape(i) for i in items))
...
def get_item_details(items, sl_entries, include_uom):
    ...
    if include_uom:
        ...
        cf_join = "left join `tabUOM Conversion Detail` ucd on ucd.parent=item.name and ucd.uom=%s" % frappe.db.escape(
            include_uom
        )
